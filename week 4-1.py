...
def _(feature_importance, features_list, plt, sns):
    ...
    top_n_features = min(20, len(features_list))
...
def _(features_list, mo, train_gini, validate_gini):
    ...
    Тренувальна вибірка: {train_gini:.4f} (0.6587) (0.7170)

    Валідаційна вибірка: {validate_gini:.4f} (0.5703) (0.5300)
...
def _(
    ...
    features_list = list(train_transactions_extracted_features.columns) + list(train_app_activity_extracted_features.columns) + list(train_communications_extracted_features.columns)
    features_list = random.sample(features_list, n_features)
    features_list = features_list + ["count_app_activity_per_user", "count_communications_per_user", "count_transactions_per_user", "days_old_ACTIVITY_DATE", "days_old_COMMUNICATION_MONTH"]
