...
def _(mo):
    ...
    Оригінальні дані збережені в pickle для швидшого завантаження
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Виокремлення тестового датасету

    Цю клітинку можна відключити. Тоді, блокнот не буде обчислювати дані, що пов'язані із тестовим датасетом, бо весь код, що працює із тестовими даними, відокремлений від решти коду.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Тестування генерації фічі

    Було перевірено всі фічі і виявлено ті, що працюють занадто довго або споживають забагато пам'яті
    """)
    return
...
def _(mo):
    mo.md(r"""
    Саме цей крок триває дуже довго. Тому, згенеровані дані зберігаються для подальшого використання.
    """)
    return
...
def _(mo):
    ...
    Об'єднання всіх згенерованих даних та заповнення порожніх та проблемних клітинок
    """)
    return
...
def _(mo):
    ...
    Експорт згенерованих даних для швидкого старту на етапі автоматичного вибору фіч
    """)
    return
...
@app.cell(disabled=True)
def _(validate_df):
    validate_df.to_pickle("validate_full.pkl")
    return


@app.cell(disabled=True)
def _(test_df):
    test_df.to_pickle("test_full.pkl")
    return


@app.cell(disabled=True)
def _(features_list, json):
    def save_json(object, filename):
        with open(filename, 'w', encoding="UTF-8") as fout:
            json.dump(object, fout, ensure_ascii=False)

    save_json(features_list, "features_list_full.json")
    return
